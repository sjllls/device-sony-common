...
struct soc_led_color_mapping soc_sony_leds[3] = {
    {15, RED_LED},
...
static int set_battery_soc_leds(int soc)
{
    ...
    for (i = 0; i < (int)ARRAY_SIZE(soc_sony_leds); i++) {
        if (soc <= soc_sony_leds[i].soc)
            break;
    }
    color = soc_sony_leds[i].color;
    if (old_color != color) {
        set_tricolor_led(0, old_color);
        set_tricolor_led(1, color);
        old_color = color;
        LOGV("soc = %d, set led color 0x%x\n", soc, soc_sony_leds[i].color);
    }
    return 0;
}
